...
practice. `status: "in-progress"` — used by 6 of 10 projects (Boston civic
data, EmberBrief, Pipeline Dashboard, Seasonal Color Palette, Vendor
intelligence feed, Sable) — renders visually identical to `"complete"`. There
is no label, dot, or other indicator anywhere on the card.
