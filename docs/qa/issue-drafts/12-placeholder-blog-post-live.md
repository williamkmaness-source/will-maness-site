...
**Update (2026-08-30 re-check): still reproduces.** `content/writing/where-the-data-moat-lives.mdx` is unchanged — still a one-line placeholder body, still syndicated in full (title + dek) in `/feed.xml` and `/sitemap.xml`, confirmed against today's `main`. The post is now dated almost 5 months in the past relative to today.

**Update (2026-09-06 re-check): still reproduces.** Unchanged in source;
still syndicated in full in `feed.xml` on a fresh `pnpm build && pnpm
start`. The post is now over five months old relative to today, one click
from the homepage the entire time.
