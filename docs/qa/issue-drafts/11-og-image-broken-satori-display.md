...
**Update (2026-09-06 re-check): still reproduces, byte-for-byte.** Third
consecutive weekly pass. `main` still unchanged since 08-01. Re-ran
`pnpm build && pnpm start`, hit `/opengraph-image` directly: identical
Satori `display: flex` error in the server log. This one-line fix has now
been sitting ready across three weeks and three QA PRs (#248, #249, this
one) with zero review action.
