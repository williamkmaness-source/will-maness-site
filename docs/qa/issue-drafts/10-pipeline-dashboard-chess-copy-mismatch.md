...
**Update (2026-08-23 re-check): still unfixed, still not filed as a real
issue.** Unchanged since first drafted. This pass found the mismatch runs
deeper than described above — three different files disagree with each
other and with the MDX copy on which pipelines are tracked (a stale "and
chess" comment in `PipelineStatusService.ts` that doesn't match its own
SQL two lines below it, and a no-DB fallback in `route.ts` that omits
`ember`, a third distinct shape from the DB-connected return value). Full
writeup: `docs/qa/2026-08-23-hiring-manager-qa.md`, finding 15 — that
finding supersedes this draft with the complete picture and updated
suggested fix; recommend filing 15 instead of this narrower version.
...
**Update (2026-08-30 re-check):** superseded by draft 15, which folds this in with the full four-sources-of-truth picture. Still not filed as a real issue; recommend filing 15 instead of this one.

**Update (2026-09-06 re-check):** still superseded by draft 15; unchanged in source.
