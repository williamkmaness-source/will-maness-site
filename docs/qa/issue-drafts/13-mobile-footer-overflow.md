...
**Update (2026-09-06 re-check): still reproduces.** `Footer.tsx` unchanged
in source; the same `flex justify-between items-center` row with no
`flex-wrap` is still there.
