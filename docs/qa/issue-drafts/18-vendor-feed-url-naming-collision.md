...
**Update (2026-08-30 re-check): still reproduces.** Both `/work/vendor-feed` (MDX narrative) and `/work/vendor_feed` (live dashboard, kept per #234's fix rather than deleted) are still live and one character apart.

**Update (2026-09-06 re-check): still reproduces.** Both routes still live,
unchanged.
