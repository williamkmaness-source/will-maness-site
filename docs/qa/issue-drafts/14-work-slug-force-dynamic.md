...
**Update (2026-08-30 re-check): still reproduces.** `src/app/work/[slug]/page.tsx` still sets `export const dynamic = "force-dynamic"` alongside its own `generateStaticParams`; `pnpm build`'s route table still shows `ƒ /work/[slug]`. Unchanged from last week.

**Update (2026-09-06 re-check): still reproduces.** Today's `pnpm build`
route table still shows `ƒ /work/[slug]` next to `● /writing/[slug]`.
Source unchanged.
